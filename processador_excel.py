...
def gerar_lista_aprovados(aprovados, exibir_mensagem, salvar_automaticamente=False):
    if salvar_automaticamente:
        with open("numeros.txt", "w") as f:
            for item in aprovados:
                f.write(f"{item}\n")
        exibir_mensagem("Nenhum erro detectado. Lista de aprovados gerada automaticamente em 'numeros.txt'.")
    else:
...
def analisar_primeira_coluna(file_path, exibir_mensagem=None):
    ...
    if erros:
        # Apenas cria o workbook e salva se houver erros
    ...
    # Gera lista de aprovados automaticamente se não houver erros
    gerar_lista_aprovados(aprovados, exibir_mensagem, salvar_automaticamente=(len(erros) == 0))
