...
def localizar_similaridades(df):
    ...
    for i in range(len(df_final) - 1, 0, -1):
...
def filtrar_df1_por_df_final():
    ...
    #df1_filtrado.to_excel('df1_filtrados.xlsx', index=False)
...
def filtrar_df2_por_df_final():
    ...
    #df2_filtrado.to_excel('df2_filtrados.xlsx', index=False)  
...
def processar_planilhas():
    ...
    df_novo = filtrar_df1_por_df_final()
    df2_novo = filtrar_df2_por_df_final()

    df_combinado = pd.concat([df_novo, df2_novo], axis=1)
